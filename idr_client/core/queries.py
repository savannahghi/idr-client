...
list_table_data = f"""
select * from {config.db_name}.etl_client_trace;
"""
...
select d.DOB,
    ...
    CAST(
        coalesce(date_first_enrolled_in_care, min(hiv.visit_date)) as Date
    ) as RegistrationDate,
    case
        max(hiv.entry_point)
    ...
    least(
        ifnull(
            ...
            reg.date_started + interval rand() * 1000 year
        ),
        reg.date_started
    ) as StartARTDate,
    ...
    GREATEST(
        COALESCE(
            hiv.date_last_modified,
            disc.date_last_modified,
            reg.date_last_modified
        ),
        COALESCE(
            disc.date_last_modified,
            reg.date_last_modified,
...
from {config.db_name}.etl_hiv_enrollment hiv
    join {config.db_name}.etl_patient_demographics d on d.patient_id = hiv.patient_id
    left outer join (
        select d.patient_id,
            coalesce(
                max(date(d.effective_discontinuation_date)),
                max(date(d.visit_date))
            ) as ExitDate,
            (
                case
        ...
        from {config.db_name}.etl_patient_program_discontinuation d
        where d.program_name = ' HIV '
        group by d.patient_id
    ) disc on disc.patient_id = hiv.patient_id
    left outer join (
        select e.patient_id,
            ...
            case
                enr.previous_art_use
                when 1 then ' Yes '
                when 0 then ' No '
            end as PreviousARTUse,
        ...
        from (
                select e.patient_id,
                    ...
                    GREATEST(
                        COALESCE(p.date_created, ph.date_created),
                        COALESCE(ph.date_created, p.date_created)
                    ) as date_created,
                    GREATEST(
                        COALESCE(p.date_last_modified, ph.date_last_modified),
                        COALESCE(ph.date_last_modified, p.date_last_modified)
                    ) as date_last_modified,
                    e.provider as provider
                from {config.db_name}.etl_drug_event e
                    join {config.db_name}.etl_patient_demographics p on p.patient_id = e.patient_id
                    left join {config.db_name}.etl_pharmacy_extract ph on ph.patient_id = e.patient_id
                    and is_arv = 1
                where e.program = ' HIV '
            ...
            inner join (
                select e.patient_id as patient_id,
                    mid(
                        max(concat(e.visit_date, e.transfer_in_date)),
                        11
                    ) as transfer_in_date,
                    case
                        mid(max(concat(e.visit_date, e.arv_status)), 11)
                        when 1 then ' Yes '
                        when 0 then ' No '
                    end as previous_art_use,
                    concat_ws(
                        ' | ',
                        NULLIF(
                            case
                                mid(max(concat(pre.visit_date, pre.PMTCT)), 11)
                                when 1065 then ' PMTCT '
                            end,
                            ''
                        ),
                        NULLIF(
                            case
                                mid(max(concat(pre.visit_date, pre.PEP)), 11)
                                when 1 then ' PEP '
                            end,
                            ''
                        ),
                        NULLIF(
                            case
                                mid(max(concat(pre.visit_date, pre.PrEP)), 11)
                                when 1065 then ' PrEP '
                            end,
                            ''
                        ),
                        NULLIF(
                            case
                                mid(max(concat(pre.visit_date, pre.HAART)), 11)
                                when 1185 then ' HAART '
                            end,
                            ''
                        )
                    ...
                    concat_ws(
                        ...
                        NULLIF(
                            case
                                mid(
                                    max(concat(pre.visit_date, pre.PMTCT_regimen)),
                                    11
                                )
                        ...
                        NULLIF(
                            case
                                mid(max(concat(pre.visit_date, pre.PEP_regimen)), 11)
                                when 164968 then ' AZT / 3TC / DTG '
                                when 164969 then ' TDF / 3TC / DTG '
                                when 164970 then ' ABC / 3TC / DTG '
                                when 164505 then ' TDF - 3TC - EFV '
                        ...
                        NULLIF(
                            case
                                mid(
                                    max(concat(pre.visit_date, pre.PrEP_regimen)),
                                    11
                                )
                                when 164968 then ' AZT / 3TC / DTG '
                                when 164969 then ' TDF / 3TC / DTG '
                        ...
                        NULLIF(
                            case
                                mid(
                                    max(concat(pre.visit_date, pre.HAART_regimen)),
                                    11
                                )
                                when 164968 then ' AZT / 3TC / DTG '
                                when 164969 then ' TDF / 3TC / DTG '
                ...
                from {config.db_name}.etl_hiv_enrollment e
                    left join {config.db_name}.etl_pre_hiv_enrollment_art pre on e.visit_id = pre.visit_id
                    and e.patient_id = pre.patient_id
                group by e.patient_id
            ) enr on enr.patient_id = e.patient_id
            left outer join {config.db_name}.etl_patient_hiv_followup fup on fup.patient_id = e.patient_id
        group by e.patient_id
    ) reg on reg.patient_id = hiv.patient_id
    join {config.db_name}.etl_default_facility_info i
